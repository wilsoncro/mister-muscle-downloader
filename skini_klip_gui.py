...
class App:
    ...
    def _popravi_trajanje_isjecka(self, vrijeme_prije):
        ...
        zadrzati POGRESNO (izvorno, puno duze) trajanje - i to na DVA razlicita
        nacina koja razliciti playeri razlicito citaju:
          ...
          2) u UGRADJENOJ METAPODATAK OZNACI "duration" koju je yt-dlp upisao
             ...
             kopira). Neki playeri (npr. Premiere) tu oznaku ignoriraju i
             racunaju stvarno trajanje - ali VLC joj vjeruje i prikazuje
             POGRESNO "2h" iako je STRUKTURNO zaglavlje vec ispravno.
        '-map_metadata -1' rjesava (2) - eksplicitno GA NE prenosi, pa svaki
        player (ukljucujuci VLC) mora racunati stvarno trajanje iz sadrzaja."""
        ffmpeg = ffmpeg_putanja()
        if not ffmpeg:
            return
        putanja = self._zadnji_fajl(vrijeme_prije)
        ...
        try:
            rezultat = subprocess.run(
                [ffmpeg, "-y", "-i", putanja, "-map", "0", "-c", "copy",
                 "-map_metadata", "-1", "-movflags", "+faststart", privremena],
